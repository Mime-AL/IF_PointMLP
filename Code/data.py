...
def download():
    ...
    if not os.path.exists(os.path.join(DATA_DIR, 'modelnet40_ply_hdf5_2048')):
        ...
        #os.system('powershell.exe wget -Uri  %s -OutFile %s' % (www, zipfile))
        #os.system('powershell.exe mv %s %s' % (zipfile, DATA_DIR))
        #os.system('powershell.exe rm %s' % (zipfile))
...
def load_data(partition):
    ...
    all_scan = []
    print(DATA_DIR)
    for h5_name in glob.glob(os.path.join(DATA_DIR, 'reg', 'ply_data_%s*.h5'%partition)):
        print(f"h5_name: {h5_name}")
        f = h5py.File(h5_name,'r')
        data = f['data'][:].astype('float32')
        label = f['label'][:].astype('float32')
        scan = f['scan'][:].astype('float32')
        f.close()
    ...
    all_scan = np.concatenate(all_scan, axis=0)
    return all_data, all_label, all_scan
...
class ModelNet40(Dataset):
    def __init__(self, num_points, partition='train'):
        self.data, self.label,self.scan = load_data(partition)
